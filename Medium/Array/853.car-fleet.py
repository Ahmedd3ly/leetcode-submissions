...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        # Pair each car's postition with its speed
        cars = zip(position, speed)

        stack = []

        # sorted(cars)[::-1] sorts by position in descending order (closer to target)
        for pos, spd in sorted(cars)[::-1]:
            # Calculate the eta this car takes to reach the target
            eta = (target - pos) / spd

            # If stack is empty, or this car takes MORE time than the fleet ahead,
            # it cannot catch up. It forms its own fleet.
            if not stack or eta > stack[-1]:
                stack.append(eta)

            # If time <= stack[-1], it catches up to the fleet ahead.
            # We don't append it because it merges into the existing fleet ahead.
